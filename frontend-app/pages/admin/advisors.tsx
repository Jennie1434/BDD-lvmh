...
export default function Advisors() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (sellersError) {
        setError('Acces refuse ou donnees indisponibles.');
        setLoading(false);
        return;
      }
  ...
  const assignmentsBySeller = useMemo(() => {
    ...
    assignments.forEach((assignment) => {
      if (!map.has(assignment.seller_id)) map.set(assignment.seller_id, []);
      map.get(assignment.seller_id)?.push(assignment.client_id);
    });
    return map;
  }, [assignments]);
  ...
  const latestMetricsBySeller = useMemo(() => {
    ...
    metrics.forEach((metric) => {
      if (!map.has(metric.seller_id)) map.set(metric.seller_id, metric);
    });
    return map;
  }, [metrics]);
  ...
  const alertsBySeller = useMemo(() => {
    ...
    alerts.forEach((alert) => {
      if (!alert.seller_id) return;
      if (!map.has(alert.seller_id)) map.set(alert.seller_id, []);
      map.get(alert.seller_id)?.push(alert);
    });
    return map;
  }, [alerts]);
  ...
  const sellerRows = useMemo(() => {
    return sellers.map((seller) => {
      const assignedClients = assignmentsBySeller.get(seller.id) || [];
      const hotClients = assignedClients.filter((id) => clientMap.get(id)?.temperature === 'HOT').length;
      const metric = latestMetricsBySeller.get(seller.id);
      return {
        ...seller,
  ...
  const selectedSeller = sellerRows.find((seller) => seller.id === selectedId);
  ...
  const toggleStatus = async () => {
    ...
    if (updateError) {
      ...
      setSellers((prev) => prev.map((seller) => (seller.id === selectedSeller.id ? { ...seller, status: nextStatus } : seller)));
    }
    setToggleBusy(false);
  };
      ...
      <div className="seller-page">
          ...
          <div className="filter-group">
            {['all', 'active', 'inactive'].map((status) => (
              <button
                ...
                onClick={() => setStatusFilter(status as 'all' | SellerStatus)}
              >
        ...
        <div className="seller-grid">
          ...
          <section className="seller-detail panel">
            ...
            {!selectedSeller ? (
              <p className="empty-state">Selectionne un vendeur pour afficher la fiche.</p>
            ) : (
                  ...
                  <div className="alert-list">
                    {selectedAlerts.length === 0 ? (
                      <div className="alert-card">
                        ...
                        <p className="alert-detail">Aucun signal critique.</p>
                      </div>
                    ) : (
