...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            setStats({
                activeSellers: sellers?.filter((s) => s.status === 'active').length || 0,
                inactiveSellers: sellers?.filter((s) => s.status === 'inactive').length || 0,
                hotClients: clients?.filter((c) => c.temperature === 'HOT').length || 0,
                warmClients: clients?.filter((c) => c.temperature === 'WARM').length || 0,
                coldClients: clients?.filter((c) => c.temperature === 'COLD').length || 0,
                pipeline: (clients || []).reduce((sum, c) => sum + (Number(c.estimated_budget) || 0), 0),
                criticalAlerts: (alertsData || []).filter((a) => !a.resolved && a.severity === 'high').length,
            });
